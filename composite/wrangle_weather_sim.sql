...
CREATE TABLE weather_sim AS 
	...
	FROM (
		SELECT station_id, 
			AVG(CASE WHEN station_decade = '200' AND napa_decade = '2000' THEN weather_sim ELSE NULL END) AS weather_sim_2000, 
			AVG(CASE WHEN station_decade = '201' AND napa_decade = '2010' THEN weather_sim ELSE NULL END) AS weather_sim_2010
		FROM weather_sim1 
		GROUP BY station_id) t1 
	LEFT JOIN (
		SELECT station_id, 
			AVG(CASE WHEN station_decade = '201' AND napa_decade = '2010' THEN weather_sim ELSE NULL END) AS weather_sim_future 	
		FROM weather_sim2
		GROUP BY station_id) t2 
	ON t1.station_id = t2.station_id
	WHERE weather_sim_2000 IS NOT NULL AND weather_sim_2010 IS NOT NULL;  