...
CREATE TABLE chorizon (chkey BIGINT, cokey BIGINT, hzdept_r INT, hzdepb_r INT, sandtotal_r FLOAT, silttotal_r FLOAT, claytotal_r FLOAT, om_r FLOAT, awc_r FLOAT, ph1to1h2o_r FLOAT) ROW FORMAT DELIMITED FIELDS TERMINATED BY '|' STORED AS TEXTFILE;

# load data
load data inpath 'hdfs:/user/yjn214/rbda-proj/legend_no_header.txt' overwrite into table legend; 
load data inpath 'hdfs://dumbo/user/yjn214/rbda-proj/mapunit_no_header_cleaned' overwrite into table mapunit_cleaned;
load data inpath 'hdfs://dumbo/user/yjn214/rbda-proj/component_no_header_cleaned' overwrite into table component;
load data inpath 'hdfs://dumbo/user/yjn214/rbda-proj/chorizon_no_header_cleaned' overwrite into table chorizon; 
